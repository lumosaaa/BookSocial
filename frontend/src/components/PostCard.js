...
import { Avatar, Image, Rate, Tag, Typography, Dropdown, message } from 'antd';
...
import { togglePostLike, bookmarkPost, deletePost } from '../api/postApi';
...
const PostCard = ({ post: initialPost, readonly = false, hideCommentEntry = false, onDeleted, onCommentClick, }) => {
    ...
    const menuItems = [
        ...(user?.id === post.userId
            ...
            : [{ key: 'report', label: '举报', icon: _jsx(WarningOutlined, {}), onClick: () => navigate(`/report?targetId=${post.id}&targetType=1`) }]),
    ];
