...
async function toggleFollow(followerId, followingId) {
  if (followerId === followingId) {
    throw { status: 400, message: '不能关注自己' };
  }
  ...
  if (!target.length) throw { status: 404, message: '用户不存在' };
  ...
  if (existing.length) {
    ...
    await db.transaction(async (conn) => {
      await conn.query(
        'INSERT INTO user_follows (follower_id, following_id, is_mutual, created_at) VALUES (?, ?, 0, NOW())',
        [followerId, followingId]
      );
      // 检查对方是否已关注我（互关）
...
async function getFollowers(userId, page = 1, pageSize = 20) {
  ...
  const [rows] = await db.query(
    `SELECT u.id, u.username, u.avatar_url, u.bio,
            u.follower_count, uf.is_mutual, uf.created_at AS followed_at
